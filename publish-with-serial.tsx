    ...
    onHandle: async () => {
      try {
        ...
        if (currentSerial == null) {
          ...
          // 숫자/문자열 케이스 모두 매칭
          const sNum =
            typeof currentSerial === 'number' ? currentSerial : Number(String(currentSerial).trim())
        ...
        if (needsNewSerial) {
          ...
          await client
            ...
            .patch(counterId, (p) => p.setIfMissing({last: 0}).inc({last: 1}))
            .commit()

          const counterDoc = await client.getDocument<{_id: string; last: number}>(counterId)
          ...
          // 항상 number로 저장
          await client.patch(myDraftId).set({serial: next}).commit({autoGenerateArrayKeys: true})
        }
